...
The cron is `0 23 * * 1-5` — 6:00 PM EST / 7:00 PM EDT, after the close in
both halves of the year with no DST handling required.

Two things to know about Actions crons: they are best-effort and can lag by
several minutes under load, and GitHub disables scheduled workflows in
repositories with no commits for 60 days. Neither matters much here — the job
is idempotent on `(symbol, date)`, so a late, skipped or repeated run is
harmless and the next run backfills the gap from each symbol's watermark.
